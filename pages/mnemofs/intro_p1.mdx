...
What makes it different from arrays in your Random Access Memory (RAM)? Many things. But most important of them is that it is non-volatile. This means that if something is stored in it, it will stay on it even if power is no longer supplied to the device. As an additional bonus, as far as storage mediums that exist till date go, if you store even a byte at a location, say, `x`, then it will be at `x` unless deleted explicitly.
...
File systems have a lot of components, which of course, have names. Like a car, some of them are very much visible to a user and they are aware of their presence, while a others are kept under-the-hood. These components are called file system objects (fs objects).

Some of the famous ones are explained below.
...
Take it like a collection of information you want together, such that programs can work on the entire thing together. An image viewer can show you an entire image in one go if you keep all of it together. A video player can play your entire movie in one go if kept in one piece. And so on.

They are analogous to one single paper, or multiple papers (usually seen in photos of offices from a time as old as the ice age) that are kept together as they belong to the same topic.
...
Suppose you have a file in a directory that is very deep in a hierarchical system of directories. But there's this folder called "Desktop" in which, if files are kept, will be shown...on your Desktop (🤯). Now, keeping everything in the desktop would not make sense, and that file is happy where it is. And you're mostly happy with where it is as well. But you need it on your Desktop as well.

Well, copy it, duh. But then updating one does not update the other. Thus, symlinks. They are a type of file which points to the location of another file. There are two types of symlinks, but technicalities.
...
So, we glanced over the basic knowledge required to start learning about file systems. The fun and games end here, and in [Part 2](../coming_soon), it's time to dive into the depths of file systems, and file system development.

<Card icon="❱ " title="Part II" href="../coming_soon" />